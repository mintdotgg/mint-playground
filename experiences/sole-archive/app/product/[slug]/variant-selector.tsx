...
import { toPortableHref } from "@/components/app-link";
...
export function VariantSelector({ variants, selectedVariantId: _selectedVariantId }: VariantSelectorProps) {
	...
	const handleOptionSelect = (label: string, optionId: string) => {
		...
		window.location.assign(toPortableHref(`${pathname}?${params.toString()}`));
	};
	...
	useEffect(() => {
		...
		const params = firstVariant.combinations.reduce((acc, c) => {
			acc.set(c.variantValue.variantType.label, c.variantValue.value);
			return acc;
		}, new URLSearchParams());
		window.location.replace(toPortableHref(`${pathname}?${params.toString()}`));
						...
						{group.type === "color" ? (
							<>
								<div className="mb-3 flex items-center justify-between">
									...
									{selectedOption && (
										<span className="text-sm text-foreground/55">{selectedOption.value}</span>
									)}
								</div>
											...
											<button
												...
												className={cn(
													...
													isSelected
														...
														: "hover:ring-2 hover:ring-foreground/45 hover:ring-offset-2 hover:ring-offset-background",
												)}
									...
									{group.options.map((option) => {
										...
										return (
											<button
												...
												onClick={() => handleOptionSelect(group.label, option.id)}
												className={cn(
													"flex flex-col items-center rounded-lg border px-6 py-3 transition-all duration-200",
													isSelected
