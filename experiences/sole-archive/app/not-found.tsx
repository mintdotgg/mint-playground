import { ShoppingCartIcon } from "lucide-react";
import type { Metadata } from "next";
import { AppLink } from "@/components/app-link";

export const metadata: Metadata = {
	title: "Page Not Found",
	description:
		"This page doesn't exist or has been moved. Continue shopping to find what you're looking for.",
};

export default function NotFound() {
	return (
		<div
			className="flex flex-1 flex-col items-center justify-center px-4 py-24 text-center"
			style={{ minHeight: "90vh" }}
		>
			<ShoppingCartIcon className="size-16 text-foreground/45" strokeWidth={1.5} />
			<h1 className="mt-6 text-7xl font-bold">404</h1>
			<h2 className="mt-4 text-xl text-foreground/60">Page not found</h2>
			<p className="mt-2 text-sm text-foreground/60">
				This page doesn’t exist or has been moved. But our store is still open!
			</p>
			<AppLink href="/" className="store-pill mt-8">
				Continue Shopping
			</AppLink>
		</div>
	);
}
